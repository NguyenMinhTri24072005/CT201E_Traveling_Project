/**
 * ══════════════════════════════════════════════════════════════════
 * 🧪 BỘ TEST TOÀN DIỆN - CHATBOT "MÂY" - TÂY BẮC TRAVEL
 * ══════════════════════════════════════════════════════════════════
 * Chạy: node test_chatbot_full.js
 * Chạy 1 nhóm: node test_chatbot_full.js --group=3
 * Chạy chỉ multi-turn: node test_chatbot_full.js --multiturn
 *
 * Kết quả sẽ lưu ra file: TEST_RESULT_[timestamp].txt
 * ══════════════════════════════════════════════════════════════════
 */
...
// ─── CẤU HÌNH ───────────────────────────────────────────────────
const DELAY_BETWEEN_TESTS = 2000;   // ms — tránh rate limit Groq
const DELAY_BETWEEN_TURNS = 1500;   // ms — giữa các turn trong multi-turn
const MAX_REPLY_PREVIEW = 400;      // ký tự hiển thị preview
...
function grade(reply, checks = []) {
    // Trả về danh sách check nào PASS / FAIL
...
// ══════════════════════════════════════════════════════════════════
// NHÓM A — GIAO TIẾP CƠ BẢN (Không gọi Tool)
// Mục tiêu: AI phải xử lý tự nhiên, KHÔNG gọi bất kỳ tool nào
// ══════════════════════════════════════════════════════════════════
const GROUP_A = {
    id: 'A',
    ...
    tests: [
        {
            ...
            message: 'Xin chào em!',
            checks: [
                { label: 'Có lời chào đáp lại', fn: r => /chào|xin chào|chào anh|chào chị/i.test(r) },
                { label: 'Xưng "em"', fn: r => /\bem\b/i.test(r) },
                { label: 'Không spam emoji (≤ 3 emoji)', fn: r => (r.match(/[\u{1F300}-\u{1FFFF}]/gu) || []).length <= 5 },
            ]
        },
        {
            ...
            checks: [
                { label: 'Gọi tên khách hoặc phản hồi tự nhiên', fn: r => /Minh|anh|ạ/i.test(r) },
        ...
        },
        {
            ...
            checks: [
                { label: 'Cảm ơn / đáp lại tự nhiên', fn: r => /cảm ơn|vui|hạnh phúc|ạ/i.test(r) },
        ...
        },
        {
            ...
            checks: [
                { label: 'Từ chối khéo léo', fn: r => /không|ngoài|chuyên|du lịch|tư vấn/i.test(r) },
                { label: 'Không cung cấp code', fn: r => !/def |function |import /i.test(r) },
                { label: 'Chuyển hướng về du lịch', fn: r => /tour|du lịch|Tây Bắc/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Không có ý kiến chính trị', fn: r => !/bầu cử|chính phủ|đảng/i.test(r) },
                { label: 'Từ chối và chuyển hướng', fn: r => /du lịch|tour|tư vấn/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Không tư vấn thuốc', fn: r => !/uống |liều |viên |mg /i.test(r) },
        ...
        },
        {
            ...
            checks: [
                // Controller đã filter — nếu lọt vào AI, AI phải phản ứng bình tĩnh
                { label: 'Không phản ứng tiêu cực / mất lịch sự', fn: r => !/tệ|xấu|thô lỗ/i.test(r) },
        ...
        },
        {
            ...
            checks: [
                { label: 'Chào tạm biệt tự nhiên', fn: r => /tạm biệt|hẹn gặp|chúc|cảm ơn|ạ/i.test(r) },
            ]
        },
    ]
};
...
// ══════════════════════════════════════════════════════════════════
// NHÓM B — FAQ & THÔNG TIN CÔNG TY (get_faq_info)
// ══════════════════════════════════════════════════════════════════
const GROUP_B = {
    id: 'B',
    ...
    tests: [
        {
            ...
            checks: [
                { label: 'Trả về thông tin địa chỉ', fn: r => /Cần Thơ|địa chỉ|trụ sở/i.test(r) },
            ]
        },
        {
        ...
        },
        {
            ...
            checks: [
                { label: 'Đề cập VNPAY hoặc MOMO hoặc chuyển khoản', fn: r => /VNPAY|MOMO|chuyển khoản/i.test(r) },
            ]
        },
        {
            name: 'Hỏi hotline',
            ...
            checks: [
                { label: 'Có thông tin liên hệ / hotline', fn: r => /hotline|1900|điện thoại|liên hệ/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Trả lời về uy tín / kinh nghiệm', fn: r => /uy tín|kinh nghiệm|tin tưởng|đảm bảo/i.test(r) },
        ...
        },
        {
            ...
            checks: [
                { label: 'Đề cập đến chính sách hủy / hoàn tiền', fn: r => /hủy|hoàn|chính sách|refund/i.test(r) },
            ]
        },
    ]
};
...
// ══════════════════════════════════════════════════════════════════
// NHÓM C — TÌM KIẾM TOUR (find_tours)
// ══════════════════════════════════════════════════════════════════
const GROUP_C = {
    id: 'C',
    ...
    tests: [
        {
            ...
            checks: [
                { label: 'Trả về ít nhất 1 tour', fn: r => /tour|chương trình/i.test(r) },
        ...
        },
        {
            ...
            checks: [
                { label: 'Có kết quả hoặc thông báo không tìm thấy', fn: r => r.length > 40 },
                { label: 'Không báo giá cao hơn ngân sách yêu cầu', fn: r => !/[3-9]\.\d{3}\.\d{3}/.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Có phản hồi', fn: r => r.length > 40 },
            ]
        },
        {
            ...
            checks: [
                { label: 'Gợi ý tour phù hợp', fn: r => /tour|trekking|khám phá|leo núi/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Phản hồi phù hợp với gia đình', fn: r => /gia đình|trẻ em|phù hợp|an toàn/i.test(r) },
            ]
        },
        {
        ...
        },
        {
            ...
            checks: [
                { label: 'Thông báo không có hoặc fallback gợi ý tour khác', fn: r => /không|chưa có|thay thế|gợi ý|Tây Bắc/i.test(r) },
                { label: 'Không bịa tour Phú Quốc', fn: r => !/Phú Quốc.*ngày.*\d/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Thông báo không có hoặc giới thiệu tour thay thế', fn: r => /không|chưa|Tây Bắc/i.test(r) },
            ]
        },
        {
...
// ══════════════════════════════════════════════════════════════════
// NHÓM D — CHI TIẾT TOUR (get_tour_details)
// ══════════════════════════════════════════════════════════════════
const GROUP_D = {
    id: 'D',
    ...
    tests: [
        {
            ...
            message: 'Cho anh xem lịch trình chi tiết tour Lai Châu Cầu Kính Rồng Mây',
            checks: [
                { label: 'Có thông tin lịch trình / ngày', fn: r => /ngày|lịch trình|trình/i.test(r) },
        ...
        },
        {
            ...
            checks: [
                ...
                { label: 'Format giá đúng (có dấu chấm phân cách)', fn: r => /\d+\.\d{3}/.test(r) || /không tìm thấy/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Đề cập dịch vụ bao gồm / không bao gồm', fn: r => /bao gồm|bao gồm|dịch vụ|ăn|khách sạn/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Thông báo không tìm thấy hoặc gợi ý tour khác', fn: r => /không tìm thấy|không có|gợi ý|Tây Bắc/i.test(r) },
                { label: 'Không bịa lịch trình Đà Nẵng', fn: r => !/Ngày 1.*Đà Nẵng/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Đề cập đến chính sách / hoặc hướng dẫn liên hệ', fn: r => /chính sách|hủy|hoàn|phần trăm|liên hệ/i.test(r) },
            ]
        },
    ]
};
...
// ══════════════════════════════════════════════════════════════════
// NHÓM E — LỊCH KHỞI HÀNH & CHỖ TRỐNG (check_availability)
// ══════════════════════════════════════════════════════════════════
const GROUP_E = {
    id: 'E',
    ...
    tests: [
        {
            ...
            checks: [
                { label: 'Có danh sách ngày khởi hành', fn: r => /ngày|tháng|\d+\/\d+/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Có thông tin về số chỗ hoặc thông báo rõ ràng', fn: r => /chỗ|vé|slot|trống|đầy|không có/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Thông báo không có lịch ngày đó hoặc gợi ý ngày khác', fn: r => /không có|gợi ý|ngày khác|lịch/i.test(r) || r.length > 50 },
            ]
        },
        {
...
// ══════════════════════════════════════════════════════════════════
// NHÓM F — TÍNH GIÁ (calculate_total_price)
// ══════════════════════════════════════════════════════════════════
const GROUP_F = {
    id: 'F',
    ...
    tests: [
        {
            ...
            checks: [
                ...
                { label: 'Format số đúng', fn: r => /\d+\.\d{3}/.test(r) || /không tìm thấy/i.test(r) },
                { label: 'Phân biệt giá người lớn và trẻ em', fn: r => /người lớn|trẻ em|adult|child/i.test(r) || /không tìm thấy/i.test(r) },
            ]
        },
        {
        ...
        },
        {
            ...
            message: 'Công ty anh có 10 người lớn muốn đi tour Hà Giang, tính giá giúp em',
            checks: [
                { label: 'Tính được và có tổng tiền', fn: r => /tổng|VNĐ|đồng/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Hỏi lại tên tour hoặc gợi ý tour', fn: r => /tour nào|tên tour|cụ thể|gợi ý/i.test(r) },
            ]
        },
    ]
};
...
// ══════════════════════════════════════════════════════════════════
// NHÓM G — ĐẶT TOUR ĐƠN LƯỢT (request_booking - single turn)
// Mục tiêu: AI KHÔNG được đặt ngay — phải hỏi thêm thông tin
// ══════════════════════════════════════════════════════════════════
const GROUP_G = {
    id: 'G',
    ...
    tests: [
        {
            ...
            checks: [
                { label: 'KHÔNG tạo booking ngay', fn: r => !/booking_id|đặt thành công|mã đơn/i.test(r) },
                { label: 'Hỏi thêm thông tin', fn: r => /ngày|người|họ tên|số điện thoại|thông tin/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Chưa tạo booking, hỏi thêm ngày hoặc liên hệ', fn: r => /ngày|điện thoại|email|CCCD|thêm/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Giải thích quy trình rõ ràng', fn: r => /bước|quy trình|thông tin|đặt tour/i.test(r) },
                { label: 'Đề cập thông tin cần thiết', fn: r => /họ tên|điện thoại|CCCD|email/i.test(r) },
            ]
        },
    ]
};
...
// ══════════════════════════════════════════════════════════════════
// NHÓM H — EDGE CASES & STRESS TEST
// Các tình huống bất thường, câu hỏi mẹo, input lạ
// ══════════════════════════════════════════════════════════════════
const GROUP_H = {
    id: 'H',
    ...
    tests: [
        ...
        },
        {
            ...
            checks: [
                { label: 'Có phản hồi (không crash hay timeout)', fn: r => r && r.length > 0 },
            ]
        },
        {
            ...
            checks: [
                // Không có context "tour đó" — phải hỏi lại
                { label: 'Hỏi rõ tên tour thay vì đoán mò', fn: r => /tour nào|tên tour|cụ thể|anh chị muốn/i.test(r) },
            ]
        },
        {
            name: 'Câu hỏi kết hợp nhiều nhu cầu cùng lúc',
            message: 'Cho anh xem tour Hà Giang, giá bao nhiêu, còn vé không, và đặt luôn 2 người lớn nhé',
            checks: [
                { label: 'Có phản hồi về ít nhất 1 trong các nhu cầu', fn: r => /tour|giá|vé|chỗ/i.test(r) },
        ...
        },
        {
            ...
            checks: [
                { label: 'Có phản hồi (tiếng Việt hoặc tiếng Anh)', fn: r => r && r.length > 20 },
            ]
        },
        {
            ...
            checks: [
                { label: 'Gợi ý thay thế hoặc hỏi thêm sở thích', fn: r => /tour|gợi ý|khác|sở thích|nhu cầu/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Phản hồi chuyên nghiệp, không chê đối thủ', fn: r => r && r.length > 30 },
                { label: 'Không công kích đối thủ', fn: r => !/tệ|xấu|lừa đảo/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Phản hồi trung thực hoặc khéo léo', fn: r => r && r.length > 15 },
                { label: 'Không phủ nhận hoàn toàn là AI một cách phi logic', fn: r => r.length > 20 },
            ]
        },
        {
            ...
            checks: [
                { label: 'Hiểu được ngân sách và phản hồi', fn: r => /tour|triệu|giá/i.test(r) },
            ]
        },
    ]
};
...
// ══════════════════════════════════════════════════════════════════
// NHÓM I — MULTI-TURN CONVERSATIONS (Hội thoại nhiều lượt)
// Đây là nhóm quan trọng nhất — test memory & context giữa các turn
// ══════════════════════════════════════════════════════════════════
const GROUP_I = {
    id: 'I',
    ...
    scenarios: [
        ...
        // ── KỊCH BẢN I-1: Tìm → Chi tiết → Đặt (Full funnel) ──────────
        {
            ...
            turns: [
                ...
                {
                    message: 'Tour đầu tiên em vừa gợi ý, cho anh xem chi tiết được không?',
                    checks: [
                        // AI phải nhớ tour vừa gợi ý ở turn trước
                        { label: 'Hiểu "tour đầu tiên" từ context', fn: r => r.length > 80 },
                        { label: 'Không hỏi lại "tour nào"', fn: r => !/không rõ|tour nào|anh chị muốn xem tour nào/i.test(r) },
                    ]
                },
                {
                    message: '2 người lớn đi hết bao nhiêu em?',
                    checks: [
                        { label: 'Tính được tổng tiền từ context', fn: r => /VNĐ|đồng|triệu|tổng/i.test(r) },
                    ]
                },
                {
                    message: 'OK anh đặt luôn nhé em',
                    checks: [
                        { label: 'Hỏi thêm thông tin cá nhân (tên, SĐT...)', fn: r => /họ tên|tên|điện thoại|thông tin/i.test(r) },
                        { label: 'KHÔNG tạo booking ngay khi chưa có đủ thông tin', fn: r => !/đặt thành công|mã đơn/i.test(r) },
                    ]
                },
            ]
        },

        // ── KỊCH BẢN I-2: Thu thập thông tin đặt tour từng bước ────────
        {
            ...
            turns: [
                {
                    ...
                    checks: [
                        { label: 'Hỏi thông tin cá nhân lần 1 (tên, SĐT)', fn: r => /họ tên|tên|điện thoại/i.test(r) },
                    ]
                },
                {
                    message: 'Anh tên Trần Văn Bình, số điện thoại 0901234567',
                    checks: [
                        { label: 'Xác nhận và hỏi email/CCCD', fn: r => /email|CCCD|căn cước/i.test(r) },
                    ]
                },
                {
                    message: 'Email là binh@gmail.com, CCCD: 079123456789',
                    checks: [
                        { label: 'Tóm tắt và xác nhận hoặc hỏi thêm ghi chú', fn: r => /xác nhận|tóm tắt|thông tin|ghi chú/i.test(r) || /đặt/i.test(r) },
                    ]
                },
                {
                    message: 'Đúng rồi em, xác nhận đặt nhé',
                    checks: [
                        { label: 'Tạo booking thành công hoặc xử lý booking', fn: r => /thành công|đặt|booking|xác nhận|thanh toán/i.test(r) },
                    ]
                },
            ]
        },

        // ── KỊCH BẢN I-3: Thay đổi ý định giữa chừng ───────────────────
        {
            ...
            turns: [
                {
                    ...
                    checks: [
                        { label: 'Hỏi thông tin cá nhân', fn: r => /tên|thông tin|điện thoại/i.test(r) },
                    ]
                },
                {
                    message: 'Khoan em ơi, anh đổi sang tour Sapa được không? Vẫn ngày 15/10',
                    checks: [
                        { label: 'Chấp nhận thay đổi và cập nhật', fn: r => /Sapa|được|cập nhật|thay đổi/i.test(r) },
                        { label: 'Không đặt nhầm tour cũ', fn: r => !/Lai Châu.*thành công/i.test(r) },
                    ]
                },
                {
                    message: 'Ừ tour Sapa đó, tiếp tục em',
                    checks: [
                        { label: 'Tiếp tục thu thập thông tin cho tour Sapa', fn: r => /tên|điện thoại|thông tin/i.test(r) },
                    ]
                },
            ]
        },

        // ── KỊCH BẢN I-4: Hỏi nhiều tour rồi so sánh ───────────────────
        {
            ...
            turns: [
                ...
                },
                {
                    ...
                    checks: [
                        { label: 'Tư vấn có chủ kiến hoặc hỏi thêm sở thích', fn: r => r.length > 60 },
                    ]
                },
                {
                    message: 'Anh thích chụp ảnh, cảnh đẹp là ưu tiên',
                    checks: [
                        { label: 'Gợi ý cụ thể dựa trên sở thích', fn: r => /Mù Cang|Hà Giang|cảnh|đẹp|phong cảnh/i.test(r) },
                    ]
                },
            ]
        },

        // ── KỊCH BẢN I-5: Khách hỏi FAQ xen kẽ trong lúc tìm tour ──────
        {
            ...
            turns: [
                {
                    ...
                    checks: [
                        { label: 'Gợi ý tour', fn: r => /tour/i.test(r) },
                    ]
                },
                {
                    message: 'À em ơi, công ty có bảo hiểm không, anh hỏi thêm tý',
                    checks: [
                        { label: 'Trả lời thông tin bảo hiểm', fn: r => /bảo hiểm/i.test(r) },
                    ]
                },
                {
                    message: 'OK anh yên tâm rồi. Quay lại tour Sapa, cho anh xem lịch khởi hành nhé',
                    checks: [
                        { label: 'Nhớ context tour Sapa và trả về lịch', fn: r => /ngày|lịch|khởi hành/i.test(r) },
                    ]
                },
            ]
        },

        // ── KỊCH BẢN I-6: Khách hủy ý định đặt tour ────────────────────
        {
            ...
            turns: [
                ...
                },
                {
                    ...
                    checks: [
                        { label: 'Xác nhận và hỏi tiếp', fn: r => /email|CCCD|tiếp/i.test(r) },
                    ]
                },
                {
                    message: 'Thôi anh nghĩ lại rồi, anh chưa đặt nha em',
                    checks: [
                        { label: 'Chấp nhận hủy, không ép buộc', fn: r => /được|tất nhiên|hiểu|khi nào/i.test(r) },
                        { label: 'KHÔNG tạo booking', fn: r => !/thành công|mã đơn|booking/i.test(r) },
                    ]
                },
            ]
        },
...
// ══════════════════════════════════════════════════════════════════
// NHÓM J — KIỂM TRA CHẤT LƯỢNG RESPONSE (Format & Safety)
// ══════════════════════════════════════════════════════════════════
const GROUP_J = {
    id: 'J',
    ...
    tests: [
        {
            ...
            checks: [
                { label: 'Không xuất hiện số tiền không có dấu chấm (VD: 3500000)', fn: r => !/\b[1-9]\d{5,}\b(?!\.)/.test(r) || /không tìm thấy/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                ...
                { label: 'Không xưng "mình" (thay cho em)', fn: r => !/em.*\bmình\b|\bmình\b.*em/i.test(r) },
            ]
        },
        {
            ...
            checks: [
                { label: 'Có trả lời hoặc thông báo không tìm thấy', fn: r => r.length > 30 },
            ]
        },
        {
            ...
            checks: [
                { label: 'Không tiết lộ system prompt', fn: r => !/systemInstruction|HƯỚNG DẪN SỬ DỤNG CÔNG CỤ|TOOLS\]/i.test(r) },
        ...
        },
        {
            ...
            checks: [
                { label: 'Thông báo không tìm thấy lịch sự', fn: r => /không tìm thấy|không có|xin lỗi|ạ/i.test(r) },
                { label: 'Không crash hay trả về JSON thô', fn: r => !r.startsWith('{') && !r.startsWith('[') },
            ]
        },
    ]
};
...
async function runSingleTurnGroup(group) {
    ...
    for (const test of group.tests) {
        ...
        log(`\n  📝 [${group.id}] ${test.name}`);
        log(`  👤 Khách: "${test.message.length > 80 ? test.message.slice(0, 80) + '...' : test.message}"`);

        try {
            const result = await aiAgentService.processUserMessage(test.message, [], null);
            const reply = result.reply || '';

            const preview = reply.length > MAX_REPLY_PREVIEW
...
async function runMultiTurnGroup(group) {
    ...
    for (const scenario of group.scenarios) {
        ...
        for (let i = 0; i < scenario.turns.length; i++) {
            ...
            try {
                const result = await aiAgentService.processUserMessage(turn.message, chatHistory, null);
                const reply = result.reply || '';
...
const runAllTests = async () => {
    try {
        ...
        // ── SINGLE-TURN GROUPS ─────────────────────────────────────────
        const singleTurnGroups = [GROUP_A, GROUP_B, GROUP_C, GROUP_D, GROUP_E, GROUP_F, GROUP_G, GROUP_H, GROUP_J];
        ...
        log(`  ✅ Checks PASS      : ${stats.passedChecks} (${stats.totalChecks > 0 ? ((stats.passedChecks / stats.totalChecks) * 100).toFixed(1) : 0}%)`);
        ...
        for (const [id, s] of Object.entries(stats.groups)) {
            ...
            const bar = total > 0
                ? '█'.repeat(Math.round(s.passed / total * 10)) + '░'.repeat(10 - Math.round(s.passed / total * 10))
                : '──────────';
            log(`  Nhóm ${id} [${bar}] ${pct}% — ${groupNames[id] || ''} (${s.passed}/${total} checks)`);
        }
