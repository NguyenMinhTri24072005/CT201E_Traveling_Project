...
const findTourByNameHelper = async (tourName, includeLocation = false) => {
  ...
  if (includeLocation) {
    orConditions.push({
      departureLocation: { $regex: safeRegexStr, $options: "i" },
    });
  }
...
const checkDateMatch = (dbDate, searchDateStr) => {
  ...
  if (dbString.includes(searchDate) || searchDate.includes(dbString))
    return true;
  ...
  if (!isNaN(d.getTime())) {
    ...
    if (ddMMyyyy.includes(searchDate) || searchDate.includes(ddMMyyyy))
      return true;
    if (dd_MM_yyyy.includes(searchDate) || searchDate.includes(dd_MM_yyyy))
      return true;
  }
  return false;
};
const aiTools = {
  executeFindTours: async (args) => {
    try {
      ...
      const { destination, keyword, departureLocation, duration, maxPrice } =
        args || {};
  ...
  executeGetTourDetails: async (args) => {
    try {
      ...
      if (!tour)
        return {
          error: `Dạ em không tìm thấy tour nào tên '${args.tourName}'.`,
        };
  ...
  executeCheckAvailability: async (args) => {
    try {
      ...
      if (!tour)
        return { error: `Không tìm thấy tour liên quan đến '${tourName}'.` };
      if (date) {
        const exactDeparture = tour.departures.find((d) =>
          checkDateMatch(d.date, date),
        );
        if (exactDeparture) {
          return {
            tourName: tour.name,
  ...
  executeCalculatePrice: async (args) => {
    try {
      ...
      if (!departure || !departure.adultPrice)
        return { error: "Tour chưa có giá chi tiết." };
  ...
  executeRequestBooking: async (args) => {
    try {
      ...
      const isFakeData = (value) =>
        !value || FAKE_PATTERNS.some((p) => p.test(value));
      if (
        isFakeData(fullName) ||
        isFakeData(phone) ||
        isFakeData(email) ||
      ...
      const departure = tour.departures.find((d) =>
        checkDateMatch(d.date, departureDate),
      );
      if (!departure)
        return { error: `Tour không có lịch khởi hành ngày ${departureDate}.` };
      ...
      return {
        success: true,
        message:
          "Đã tạo yêu cầu đặt Tour thành công. Vui lòng thanh toán trong 24h.",
        bookingSummary: {
          tourName: tour.name,
