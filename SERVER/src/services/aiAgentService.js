...
const tools = [
    {
        functionDeclarations: [
            {
                name: "find_tours",
                description: "Tìm kiếm và gợi ý tour du lịch dựa trên nhu cầu của khách: địa điểm, sở thích, ngân sách, số ngày, điểm khởi hành.",
                parameters: {
                    type: "OBJECT",
                    properties: {
                        destination: { type: "STRING", description: "Điểm đến (VD: Sapa, Hà Giang, Mù Cang Chải)" },
                        keyword: { type: "STRING", description: "Từ khóa sở thích (VD: trekking, mạo hiểm, lãng mạn, gia đình, nghỉ dưỡng)" },
                        departureLocation: { type: "STRING", description: "Điểm khởi hành (VD: Cần Thơ, Hà Nội, TP.HCM)" },
                        duration: { type: "STRING", description: "Số ngày đi (VD: 3 ngày thì nhập '3')" },
                        maxPrice: { type: "STRING", description: "Giá tối đa bằng số nguyên (VD: dưới 3 triệu → nhập '3000000')" },
                        minPrice: { type: "STRING", description: "Giá tối thiểu bằng số nguyên (VD: trên 5 triệu → nhập '5000000')" }
                    }
                }
            },
            {
                name: "get_tour_details",
                description: "Xem chi tiết đầy đủ một tour: lịch trình từng ngày, giá vé người lớn/trẻ em, dịch vụ bao gồm, dịch vụ không bao gồm, chính sách hủy.",
                parameters: {
                    type: "OBJECT",
                    properties: {
                        tourName: { type: "STRING", description: "Tên tour cần xem. Nếu khách nói 'tour đó/tour vừa rồi', hãy điền tên tour từ lịch sử hội thoại." }
                    },
                    required: ["tourName"]
                }
            },
            {
                ...
                description: "Xem lịch khởi hành và số chỗ trống của một tour. Dùng khi khách hỏi 'có ngày nào đi?' hoặc 'ngày X còn vé không?'",
                parameters: {
                    type: "OBJECT",
                    properties: {
                        tourName: { type: "STRING", description: "Tên tour" },
                        date: { type: "STRING", description: "Ngày khởi hành cụ thể (VD: 20/10/2026). Nếu khách hỏi chung 'có ngày nào?' thì BỎ TRỐNG." }
                    },
                    required: ["tourName"]
                }
            },
            {
                ...
                description: "Tính tổng tiền tour cho nhiều người. Dùng khi khách báo số lượng người và muốn biết tổng chi phí.",
                parameters: {
                    type: "OBJECT",
                    properties: {
                        tourName: { type: "STRING", description: "Tên tour cần tính giá" },
                        adultCount: { type: "STRING", description: "Số người lớn (VD: '2')" },
                        childCount: { type: "STRING", description: "Số trẻ em (VD: '1'). Bỏ qua nếu không có." }
                    },
            ...
            },
            {
                ...
                description: "Tạo đơn đặt tour. CHỈ ĐƯỢC GỌI KHI ĐÃ CÓ ĐỦ DỮ LIỆU THẬT từ khách. TUYỆT ĐỐI KHÔNG tự điền thông tin.",
                parameters: {
                    type: "OBJECT",
                    properties: {
                        ...
                        fullName: { type: "STRING", description: "Họ tên đầy đủ của khách — phải được khách gõ rõ ràng" },
                        phone: { type: "STRING", description: "Số điện thoại của khách (10-11 số)" },
                        email: { type: "STRING", description: "Email của khách (chứa @)" },
                        cccd: { type: "STRING", description: "Số CCCD của khách" },
                        notes: { type: "STRING", description: "Ghi chú thêm (ăn chay, say xe...)" }
                    },
                    required: ["tourName", "departureDate", "adultCount", "fullName", "phone", "email", "cccd"]
                }
            },
            {
                name: "get_faq_info",
                description: "Truy vấn thông tin công ty và các câu hỏi thường gặp: địa chỉ, hotline, email, bảo hiểm, phương thức thanh toán, uy tín công ty, chính sách hủy/hoàn tiền.",
                parameters: {
                    type: "OBJECT",
                    properties: {
                        question: { type: "STRING", description: "Nội dung câu hỏi của khách" }
                    },
                    required: ["question"]
                }
            }
...
// ══════════════════════════════════════════════════════════════════
// HELPER: Chuyển chatHistory (OpenAI format) → Gemini format
// chatHistory từ client: [{ role: 'user'|'assistant', content: '...' }]
...
// ══════════════════════════════════════════════════════════════════
// HELPER: Thực thi tool theo tên
// ══════════════════════════════════════════════════════════════════
const executeTool = async (toolName, args, userId) => {
    console.log(`[TOOL CALLED] ${toolName}`, args);
    switch (toolName) {
        case "find_tours":          return await aiTools.executeFindTours(args);
        case "get_tour_details":    return await aiTools.executeGetTourDetails(args);
        case "check_availability":  return await aiTools.executeCheckAvailability(args);
        case "calculate_total_price": return await aiTools.executeCalculatePrice(args);
        case "request_booking":     return await aiTools.executeRequestBooking({ ...args, userId });
        case "get_faq_info":        return await aiTools.executeGetFaqInfo(args);
        default:
...
const aiAgentService = {
    processUserMessage: async (userMessage, chatHistory = [], userId = null) => {
        try {
            ...
            // Chuyển history sang Gemini format
            ...
            if (functionCalls && functionCalls.length > 0) {
                ...
                // Gemini yêu cầu functionResponse.response phải là object, không được là array.
                // Nếu tool trả về array → wrap vào { result: [...] }
                const responsePayload = Array.isArray(toolData)
                    ? { result: toolData }
                    : (toolData && typeof toolData === 'object' ? toolData : { result: toolData });

                // Gửi kết quả tool trở lại cho Gemini để sinh câu trả lời cuối
            ...
            if (error.status === 429) {
                return {
                    reply: "Dạ hệ thống đang có nhiều người dùng cùng lúc, anh/chị chờ em vài giây rồi nhắn lại nhé! 🌸",
                    thinkingType: "rate_limit"
                };
            }
            if (error.status === 401 || error.status === 403) {
                return {
                    reply: "Dạ hệ thống đang bảo trì, anh/chị vui lòng liên hệ hotline để được hỗ trợ nhé ạ!",
