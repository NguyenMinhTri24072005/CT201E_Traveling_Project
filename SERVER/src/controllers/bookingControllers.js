...
const bookingControllers = {
  createBooking: async (req, res) => {
    try {
      ...
      if (!departure)
        return res
          .status(404)
          .json({ success: false, message: "Không tìm thấy ngày khởi hành" });
      ...
      if (departure.availableslots < totalTickets) {
        return res
          ...
          .json({
            success: false,
            message: `Chỉ còn ${departure.availableslots} chỗ trống.`,
          });
      }
      let secureTotalPrice = 0;
      const secureTickets = tickets.map((ticket) => {
        let actualPrice = 0;
        if (ticket.ticketType === "Người lớn")
          actualPrice = departure.adultPrice;
        else if (ticket.ticketType === "Trẻ em")
          actualPrice = departure.childPrice;
        else if (ticket.ticketType === "Em bé")
          actualPrice = departure.babyPrice;
      ...
      const tourInfo = await Tour.findById(tourId).populate(
        "createdBy",
        "commissionRate",
      );
    ...
    try {
      const bookings = await Booking.find()
        .populate({
          path: "tour",
          populate: {
            path: "partner createdBy",
            select: "fullname email commissionRate",
          },
        })
  ...
  getPartnerBookings: async (req, res) => {
    try {
      ...
      const tours = await Tour.find({
        $or: [{ createdBy: partnerId }, { partner: partnerId }],
      });
  ...
  updateBookingStatus: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Đã cập nhật trạng thái", data: updatedBooking });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  },
  cancelBooking: async (req, res) => {
    try {
      ...
      if (booking.status !== "pending_payment") {
        return res
          .status(400)
          .json("Đơn hàng đã được xử lý hoặc thanh toán, không thể tự hủy.");
      }
      booking.status = "cancelled";
      await booking.save();
      ...
      res
        .status(200)
        .json({ message: "Đã hủy đơn thành công!", data: booking });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  },
  ...
  getPartnerStats: async (req, res) => {
    try {
      ...
      let pipeline = [
        ...
        {
          $match: {
            "tourData.createdBy": new mongoose.Types.ObjectId(partnerId),
          },
        },
      ];
      if (tourId && tourId !== "All") {
        pipeline.push({
          $match: { "tourData._id": new mongoose.Types.ObjectId(tourId) },
        });
      }
      let groupPipeline = [
        ...pipeline,
          ...
          $group: {
            ...
            netIncome: {
              $sum: {
                $subtract: [
                  "$totalprice",
                  { $multiply: ["$totalprice", commissionRate / 100] },
                ],
              },
            },
            totalBookings: { $sum: 1 },
            ...
            totalAdminProfit: {
              $sum: {
                $multiply: [
                  ...
                  {
                    $divide: [
                      { $ifNull: ["$partnerData.commissionRate", 10] },
                      100,
                    ],
                  },
                ],
