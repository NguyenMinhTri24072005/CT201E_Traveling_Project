...
const authControllers = {
  ...
  deleteUser: async (req, res) => {
    try {
      ...
      if (!user) return res.status(404).json("Không tìm thấy User!");
      if (user.role === "Admin")
        return res.status(403).json("Không thể xóa tài khoản Admin!");
  ...
  updateCommission: async (req, res) => {
    try {
      ...
      if (commissionRate < 0 || commissionRate > 100) {
        return res
          .status(400)
          .json({ message: "Tỷ lệ hoa hồng phải từ 0 đến 100." });
      }
      const updatedUser = await User.findByIdAndUpdate(
        userId,
        { commissionRate },
      ...
      res
        .status(200)
        .json({
          message: `Đã cập nhật chiết khấu thành ${commissionRate}%`,
          user: updatedUser,
        });
    } catch (error) {
      res.status(500).json({ error: error.message });
