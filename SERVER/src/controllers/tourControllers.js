...
const generateCleanTextForEmbedding = async (tourData) => {
  ...
  if (tourData.departures && tourData.departures.length > 0) {
    const validPrices = tourData.departures
      .map((d) => d.adultPrice)
      .filter((p) => p != null && p > 0);
  ...
  const itineraryText =
    tourData.itinerary && tourData.itinerary.length > 0
      ? tourData.itinerary
          .map((item) => `${item.day}: ${item.content}`)
          .join(". ")
      : "";
  const rawText = `
...
const tourControllers = {
  getAllTours: async (req, res) => {
    try {
      ...
      if (location) {
        ...
        const destLocs = await Location.find({
      ...
      if (search) {
        const searchLocs = await Location.find({
  ...
  createTour: async (req, res) => {
    try {
      ...
      if (typeof tourData.departures === "string")
        tourData.departures = JSON.parse(tourData.departures);
      ...
      if (
        ...
        req.files["itineraryImages"].length > 0
      ) {
        ...
        imageMap.forEach((itinIndex, arrayIndex) => {
          if (tourData.itinerary[itinIndex]) {
            tourData.itinerary[itinIndex].image =
              `/uploads/tours/${itinFiles[arrayIndex].filename}`;
          }
        });
      }
  ...
  bulkChangeTourStatus: async (req, res) => {
    try {
      ...
      if (!Array.isArray(tourIds) || tourIds.length === 0) {
        return res
          .status(400)
          .json({ message: "Không có Tour nào được chọn." });
      }
      ...
      res
        .status(200)
        .json({ message: `Đã xử lý thành công ${tourIds.length} Tour.` });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  },
  getRecommendations: async (req, res) => {
    try {
      ...
      const recommendations = await Tour.find({
        ...
        .select(
          "name duration departureLocation image images departures averageRating totalReviews",
        )
