...
const reviewControllers = {
  createReview: async (req, res) => {
    try {
      ...
      if (!tourId || !bookingId || !rating || !comment) {
        return res
          .status(400)
          .json({ message: "Vui lòng cung cấp đủ thông tin đánh giá!" });
      }
      ...
      if (currentStatus !== "completed") {
        return res
          .status(400)
          .json({
            message: `Bạn chỉ có thể đánh giá khi đơn hàng đã Hoàn thành (Hiện tại: ${booking.status})`,
          });
      }
      ...
      if (existingReview) {
        return res
          .status(400)
          .json({ message: "Bạn đã đánh giá chuyến đi này rồi!" });
      }
      ...
      res
        .status(200)
        .json({ message: "Cảm ơn bạn đã đánh giá!", review: newReview });
    } catch (error) {
  ...
  updateReview: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Cập nhật đánh giá thành công!", review });
    } catch (error) {
