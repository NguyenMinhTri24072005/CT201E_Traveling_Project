...
export default function TourDetail() {
    ...
    useEffect(() => {
        const fetchTourAndReviews = async () => {
            try {
                ...
                if (tourResponse.data.departures && tourResponse.data.departures.length > 0) {
    ...
    if (loading) return <h2 className="tour-loading-msg">⏳ Đang tải thông tin tour...</h2>;
    if (!tour) return <h2 className="tour-error-msg">❌ Không tìm thấy Tour!</h2>;
            ...
            <div className="tour-hero-section">
                <div className="tour-gallery-col">
                    <div className="main-image-container">
                        <img
                            src={displayImg ? `${IMAGE_URL}${displayImg}` : '/placeholder.jpg'}
                            alt={tour.name}
                ...
                <div className="tour-booking-col">
                    ...
                    <div className="tour-meta-info">
                        ...
                        <span>📍 KH: {tour.departureLocation}</span>
                        <span>🏷️ Mã: {tour.code}</span>
                    </div>

                    <hr className="tour-divider" />

                    <h3 className="booking-section-title">📅 Chọn ngày khởi hành:</h3>
                        ...
                        <div>
                            ...
                            >
                                {tour.departures.map(dep => (
                                    <option key={dep._id} value={dep._id} disabled={dep.availableslots <= 0}>
                                        {dep.date} (Về: {dep.returnDate}) - {dep.availableslots <= 0 ? 'HẾT CHỖ' : `Còn ${dep.availableslots} chỗ`}
                                    </option>
                                ))}
                            </select>
                            ...
                            {selectedDeparture && (
                                <div className="departure-details-box">
                                    <p className="departure-transport">🚙 Phương tiện: <strong>{selectedDeparture.transport}</strong></p>
                                    ...
                                    <div className="price-sub-row">
                                        ...
                                        <span>{selectedDeparture.childPrice?.toLocaleString('vi-VN')}₫</span>
                                    </div>

                                    <div className="price-sub-row">
                                        <span>Em bé ({'<'}5t):</span>
                                        <span>{selectedDeparture.babyPrice?.toLocaleString('vi-VN')}₫</span>
                                    </div>
                                </div>
                            )}
                        </div>
                    ) : (
                        <p style={{ color: 'red', fontStyle: 'italic', marginBottom: '20px' }}>Tour hiện chưa có lịch khởi hành.</p>
                    )}

                    <button
                        onClick={handleBookNow}
                        disabled={!selectedDeparture || selectedDeparture.availableslots <= 0}
                        className={`btn-book-now ${(!selectedDeparture || selectedDeparture.availableslots <= 0) ? 'disabled' : 'active'}`}
                    >
                        {(!selectedDeparture || selectedDeparture.availableslots <= 0) ? 'HẾT CHỖ' : 'ĐẶT TOUR NGAY'}
                    </button>
                    ...
                    <div className="provider-info-box">
                        ...
                        <div className="provider-content">
                            ...
                            <div>
                                ...
                                <button
                                    onClick={() => navigate(`/shop/${typeof tour.createdBy === 'object' ? tour.createdBy._id : tour.createdBy}`)}
            ...
            <div className="tour-content-section">
                <div className="tour-content-left">
                    <div className="content-block">
                        <h2 className="content-title">🌟 Điểm nhấn hành trình</h2>
                        <ul className="highlights-list">
                            {tour.highlights && tour.highlights.map((hl, index) => (
                                <li key={index}>{hl}</li>
                            ))}
                        </ul>
                    </div>
                    ...
                    <div className="content-block">
                        <h2 className="content-title">🗺️ Lịch trình chi tiết</h2>
                        {tour.itinerary && tour.itinerary.map((day, index) => (
                            <div key={index} className="itinerary-day-box-v2">
                                <div className="itinerary-left">
                                    ...
                                    {day.meals && <p className="itinerary-meals">🍽️ Bữa ăn: {day.meals}</p>}
                    ...
                    <div className="reviews-section">
                        <h2 className="reviews-title">
                            💬 Đánh giá từ Khách hàng ({tour.totalReviews || 0})
                        </h2>

                        {reviews.length === 0 ? (
                            <div className="reviews-empty">
                                <span className="reviews-empty-icon">🏝️</span>
                                <strong>Chưa có đánh giá nào.</strong> <br /> Hãy là những người đầu tiên trải nghiệm và chia sẻ cảm nhận về chuyến đi này nhé!
                            </div>
                        ) : (
                                ...
                                {reviews.map(review => (
                                    <div key={review._id} className="review-card">
                                        <div className="review-avatar-box">
                                            <img
                                                src={review.user?.avatar && !review.user.avatar.includes('default') ? `${IMAGE_URL}${review.user.avatar}` : "https://via.placeholder.com/60"}
                                                alt="avatar"
                                        ...
                                        <div className="review-content">
                                            <div className="review-header">
                                                <strong className="reviewer-name">{review.user?.fullname || 'Khách hàng'}</strong>
                    ...
                    {recommendedTours.length > 0 && (
                        <div className="recommendation-section">
                            <h2 className="recommendation-title">🌟 Bạn Có Thể Quan Tâm</h2>
                            <p className="recommendation-desc">
                                Dựa trên sự quan tâm của bạn, Tây Bắc Travel gợi ý những hành trình tuyệt vời sau:
                            </p>
