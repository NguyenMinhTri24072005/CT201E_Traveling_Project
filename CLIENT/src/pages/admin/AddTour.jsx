    ...
    const fetchData = async () => {
      try {
        ...
        if (locRes.data.length > 0)
          setBasicInfo((prev) => ({ ...prev, location: locRes.data[0]._id }));
        if (catRes.data.length > 0)
          setBasicInfo((prev) => ({ ...prev, category: catRes.data[0]._id }));
      } catch (error) {
    ...
    <div className="admin-page add-tour-container">
      ...
      <form onSubmit={handleSubmit} className="add-tour-form">
        ...
        <div className="form-group" style={{ marginBottom: "20px" }}>
          ...
          {imageFiles.length > 0 && (
            <small style={{ display: "block", color: "green" }}>
              {imageFiles.length} ảnh đã được chọn.
            </small>
          )}
        </div>
        {}
        ...
        <button type="submit" disabled={loading} className="btn-submit-tour">
          {loading ? (
            <>
              <i className="fa-solid fa-spinner fa-spin"></i> Đang lưu dữ
              liệu...
            </>
          ) : (
            <>
              <i className="fa-solid fa-floppy-disk"></i> LƯU TOUR MỚI
            </>
          )}
        </button>
      </form>
