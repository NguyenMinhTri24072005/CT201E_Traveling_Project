...
import './AdminProfile.css'; /* 👉 Kế thừa Input Styles */
import './ShopSettings.css'; /* 👉 CSS Tùy chỉnh ảnh bìa */

export default function ShopSettings() {
    const { user, updateUser } = useAuth(); 
    const [loading, setLoading] = useState(false);
    const [formData, setFormData] = useState({
        shopName: '', shopDescription: '', shopPolicies: ''
    });
    ...
    return (
        <div className="admin-page fade-in shop-settings-container">
            <h2 className="admin-title">🏪 Thiết Lập Gian Hàng</h2>
            <p className="shop-settings-subtitle">
                Gian hàng chuyên nghiệp sẽ giúp khách hàng tin tưởng và đặt Tour của bạn nhiều hơn.
            </p>

            <form onSubmit={handleSubmit} className="shop-settings-form">
                
                <div className="shop-form-group">
                    <label className="shop-form-label">Ảnh Bìa Gian Hàng (Cover Image)</label>
                    <div 
                        className="cover-upload-box"
                        style={{ backgroundImage: previewCover ? `url(${previewCover})` : 'none' }}
                    >
                        {!previewCover && <span className="cover-upload-placeholder">Bấm vào để tải ảnh lên (Tỉ lệ 16:9)</span>}
                        <input 
                            type="file" 
                            accept="image/*" 
                ...
                <div className="shop-form-group">
                    <label className="shop-form-label">Tên Thương Hiệu (Hiển thị cho khách)</label>
                    <input 
                        required 
                        type="text" 
                ...
                <div className="shop-form-group">
                    <label className="shop-form-label">Giới thiệu về bạn / Công ty của bạn</label>
                    <textarea 
                        required 
                        rows="4" 
                ...
                <div className="shop-form-group">
                    <label className="shop-form-label">Chính sách riêng của Gian hàng</label>
                    <textarea 
                        rows="3" 
                        value={formData.shopPolicies} 
                        onChange={(e) => setFormData({...formData, shopPolicies: e.target.value})} 
