...
export default function EditTour() {
  ...
  const addItineraryDay = () =>
    setItinerary([...itinerary, { day: "", meals: "", content: "" }]);
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append(
      "duration",
      `${basicInfo.durationDays} Ngày ${basicInfo.durationNights} Đêm`,
    );
  ...
  if (fetching)
    return (
      <h2 style={{ textAlign: "center", marginTop: "50px", color: "#2c3e50" }}>
        <i className="fa-solid fa-spinner fa-spin"></i> Đang tải dữ liệu Tour...
      </h2>
    );
  return (
    <div className="admin-page add-tour-container">
      <h2 className="admin-title">
        <i className="fa-solid fa-pen-to-square"></i> Cập Nhật Tour
      </h2>
      <form onSubmit={handleSubmit} className="add-tour-form">
        ...
        <div className="current-image-preview">
          ...
          <br />
          <small>Chọn các ảnh khác nếu muốn THAY THẾ toàn bộ ảnh cũ:</small>
          <br />
          <input
            type="file"
            multiple
          ...
          {imageFiles.length > 0 && (
            <small style={{ display: "block", color: "green" }}>
              {imageFiles.length} ảnh mới sẽ ghi đè ảnh cũ.
            </small>
          )}
        </div>
        ...
        {departures.map((dep, index) => (
          <div key={index} className="dynamic-block departure-block">
            ...
            <div className="form-grid-3">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleDepartureChange(index, "returnDate", e.target.value)
                  }
                  required
                  className="form-input"
                />
              ...
              <div className="form-group">
                ...
                <select
                  value={dep.transport}
                  onChange={(e) =>
                    handleDepartureChange(index, "transport", e.target.value)
                  }
                  required
                  className="form-input"
                >
                  {TRANSPORT_METHODS.map((trans) => (
            ...
            <div className="form-grid-4">
              <div className="form-group">
                <small>Giá NL (12t+):</small>
                <input
                  type="number"
                  value={dep.adultPrice}
                  onChange={(e) =>
                    handleDepartureChange(index, "adultPrice", e.target.value)
                  }
                  required
                  className="form-input"
                />
              ...
              <div className="form-group">
                <small>Giá TE (5-11t):</small>
                <input
                  type="number"
                  value={dep.childPrice}
                  onChange={(e) =>
                    handleDepartureChange(index, "childPrice", e.target.value)
                  }
                  required
                  className="form-input"
                />
              ...
              <div className="form-group">
                <small>Giá EB ({"<"}5t):</small>
                <input
                  type="number"
                  value={dep.babyPrice}
                  onChange={(e) =>
                    handleDepartureChange(index, "babyPrice", e.target.value)
                  }
                  required
                  className="form-input"
                />
              ...
              <div className="form-group">
                <small>Số chỗ tối đa:</small>
                <input
                  type="number"
                  value={dep.maxslots}
                  onChange={(e) =>
                    handleDepartureChange(index, "maxslots", e.target.value)
                  }
                  required
                  className="form-input"
                />
        ...
        <button type="button" onClick={addDeparture} className="btn-add-item">
          + Thêm đợt khởi hành
        </button>
        ...
        {itinerary.map((itin, index) => (
          <div key={index} className="dynamic-block">
            <div className="itinerary-day-input">
              <input
                type="text"
                placeholder="Tiêu đề (VD: Ngày 1 - Hà Nội - Sapa)"
                value={itin.day}
                onChange={(e) =>
            ...
            <div className="form-group">
              <textarea
                placeholder="Nội dung hoạt động chi tiết..."
                value={itin.content}
                onChange={(e) =>
            ...
            <div className="form-group" style={{ marginTop: "10px" }}>
              ...
              >
                ...
                <input
                  type="file"
                  onChange={(e) =>
                    handleItinImageChange(index, e.target.files[0])
                  }
                  accept="image/*"
                />
              </div>
        ...
        <button type="submit" disabled={loading} className="btn-update-tour">
          {loading ? (
            <>
              <i className="fa-solid fa-spinner fa-spin"></i> Đang lưu thay
              đổi...
            </>
          ) : (
            <>
              <i className="fa-solid fa-floppy-disk"></i> CẬP NHẬT TOUR
            </>
          )}
        </button>
      </form>
