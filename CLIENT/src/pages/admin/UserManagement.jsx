...
export default function UserManagement() {
    ...
    const handleCreateUser = async () => {
        const { value: formValues } = await Swal.fire({
            title: '<h3 class="swal-user-title">➕ THÊM THÀNH VIÊN</h3>',
            html: `
        ...
        if (formValues) {
            try {
                ...
                Swal.fire('Lỗi', err.response?.data?.message || 'Không thể tạo', 'error');
            }
        }
    };
    ...
    const handleToggleLock = async (user) => {
        if (user.role === 'Admin') return Swal.fire('Lỗi', 'Không thể khóa tài khoản Admin!', 'error');
    ...
    const handleEditCommission = async (partner) => {
        const { value: rate } = await Swal.fire({
            ...
            preConfirm: () => {
                ...
                if (val < 0 || val > 100 || val === '') {
                    Swal.showValidationMessage('Tỷ lệ phải nằm trong khoảng từ 0 đến 100!');
                    return false;
                }
                return val;
            }
        ...
        if (rate !== undefined) {
            try {
                await axiosClient.put(`/auth/users/${partner._id}/commission`, { commissionRate: Number(rate) });
                Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: `Đã cập nhật chiết khấu thành ${rate}%`, showConfirmButton: false, timer: 2000 });
                fetchUsers();
            } catch (error) {
    ...
    return (
        <div className="admin-page fade-in">
            <div className="user-management-header">
                <h2 className="admin-title user-management-title">👥 Quản Lý Thành Viên</h2>
            ...
            <div className="table-container">
                <table className="admin-table">
                    ...
                    <tbody>
                        {users.length === 0 ? (
                            <tr><td colSpan="6" style={{ textAlign: 'center', padding: '30px' }}>Chưa có người dùng nào!</td></tr>
                        ) : (
                            users.map(u => (
                                <tr key={u._id} className={`user-row ${u.isLocked ? 'locked' : ''}`}>
                                    
                                    <td>
                                        <div className="user-profile-cell">
                                            <img 
                                                src={u.avatar && !u.avatar.includes('default') ? `${IMAGE_URL}${u.avatar}` : "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png"} 
                                                alt="avt" 
                                                className="user-avatar-img"
                                            />
                                            {u.fullname || <span className="user-fullname-empty">Chưa cập nhật</span>}
                                        </div>
                                    </td>
                                    
                                    <td>
                                        <div className="user-contact-email">✉️ {u.email}</div>
                                        <div className="user-contact-phone">📞 {u.phone || 'N/A'}</div>
                                    </td>
                                    ...
                                    <td className="commission-cell">
                                        {u.role === 'Partner' ? (
                                            <div className="commission-wrapper">
                                                <strong className="commission-rate">{u.commissionRate ?? 10}%</strong>
                                                <button 
                                    ...
                                    <td>
                                        <span className={`status-badge ${u.isLocked ? 'locked' : 'active'}`}>
