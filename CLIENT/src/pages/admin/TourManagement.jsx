...
export default function TourManagement() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const allTours = Array.isArray(tourRes.data.tours)
        ? tourRes.data.tours
        : [];
      if (user?.role === "Admin") {
  ...
  const processBulkAction = async (targetStatus, actionName, confirmColor) => {
    if (selectedTours.length === 0)
      return Swal.fire("Thông báo", "Vui lòng chọn ít nhất 1 Tour!", "info");
    ...
    if (confirm.isConfirmed) {
      try {
        await axiosClient.put("/tours/admin/bulk-status", {
          tourIds: selectedTours,
          status: targetStatus,
        });
        Swal.fire(
          "Thành công!",
          `Đã ${actionName.toLowerCase()} ${selectedTours.length} Tour.`,
          "success",
        );
        setSelectedTours([]);
        fetchAllData();
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire(
            "Đã xóa!",
            response.data.message || "Tour đã được loại bỏ.",
            "success",
          );
          fetchAllData();
        } catch (error) {
          Swal.fire(
            "Lỗi!",
            error.response?.data?.message || "Không thể xóa tour này.",
            "error",
          );
        }
      }
  ...
  const handleApproveTour = async (tourId, newStatus) => {
    ...
    try {
      await axiosClient.put(`/tours/${tourId}/status`, {
        status: newStatus,
        rejectReason: reason,
      });
      Swal.fire({
        ...
        position: "top-end",
        icon: "success",
        title: `Đã ${newStatus === "Approved" ? "Duyệt" : "Từ chối"}!`,
  ...
  const handleViewTourDetails = (tour) => {
    ...
    const displayCover =
      tour.images && tour.images.length > 0
        ? tour.images[0]
        : tour.image && !tour.image.includes("default")
          ? tour.image
          : null;
    const initialImage = displayCover
      ? IMAGE_URL + displayCover
      : "https://via.placeholder.com/800x400";
    let highlightsHtml = "";
    if (tour.highlights && tour.highlights.length > 0) {
      highlightsHtml = `
                <div style="margin-bottom: 20px; text-align: left;">
                    <span style="font-weight: bold; color: #2d4271; display: block; margin-bottom: 8px;"><i class="fa-solid fa-star"></i> Điểm nổi bật & Tiện ích:</span>
    ...
    if (tour.images && tour.images.length > 0) {
      galleryHtml = `
                <div class="swal-gallery-section" style="margin-bottom: 25px; text-align: left;">
                    <span style="font-weight: bold; color: #2d4271; display: block; margin-bottom: 10px;"><i class="fa-solid fa-images"></i> Thư viện ảnh (${tour.images.length}):</span>
    ...
    if (tour.departures && tour.departures.length > 0) {
      departuresHtml = `
                <div class="swal-departure-section" style="margin-bottom: 25px; text-align: left;">
                    <span style="font-weight: bold; color: #2d4271; display: block; margin-bottom: 10px;"><i class="fa-solid fa-money-bill"></i> Bảng giá & Lịch khởi hành:</span>
                ...
                <div class="swal-tour-detail-monitor" style="max-height: 85vh; overflow-y: auto; padding-right: 10px; text-align: left;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; background: #f8fafc; padding: 15px; border-radius: 10px; margin-bottom: 20px; border: 1px solid #e2e8f0;">
                        ...
                        <div>
                            ...
                            <div style="font-size: 0.8rem; color: #4a5568;"><i class="fa-solid fa-location-dot"></i> ${tour.departureLocation} ➜ ${tour.destination || ""}</div>
                        </div>
                    </div>
                    ${galleryHtml}
                    <div style="background: #fffaf0; padding: 12px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #feebc8; display: flex; justify-content: space-around; text-align: center;">
                        <div><small style="color: #7b341e;">Thời lượng</small><br/><b><i class="fa-solid fa-clock"></i> ${tour.duration}</b></div>
                        <div><small style="color: #7b341e;">Đánh giá</small><br/><b><i class="fa-solid fa-star"></i> ${tour.averageRating || 0}/5</b></div>
                    </div>
                    ${highlightsHtml}
                    ${departuresHtml}
                    <span style="font-weight: bold; color: #2d4271; display: block; margin-bottom: 15px; border-bottom: 2px solid #eee; padding-bottom: 5px;"><i class="fa-solid fa-map-location-dot"></i> Chi tiết lịch trình & Ảnh ngày:</span>
  ...
  if (loading)
    return (
      <p style={{ padding: "20px", color: "#2d4271", fontWeight: "bold" }}>
        Đang tải dữ liệu Tour...
      </p>
    );
  const isAllSelected =
    filteredTours.length > 0 && selectedTours.length === filteredTours.length;
  return (
    <div className="admin-page fade-in">
      <div className="tour-management-header">
        <h2 className="admin-title tour-management-title">
          {user?.role === "Admin" ? (
            <>
              <i className="fa-solid fa-crown"></i> Kiểm Duyệt Tour Toàn Sàn
            </>
          ) : (
            <>
              <i className="fa-solid fa-map-location-dot"></i> Quản Lý Tour Của
              Tôi
            </>
          )}
        </h2>
          ...
          {user?.role === "Admin" && selectedTours.length > 0 && (
            <div className="bulk-action-bar">
              <span className="bulk-action-text">
                Đã chọn: {selectedTours.length}
              </span>
              <button
                onClick={() =>
                  processBulkAction("Approved", "Duyệt", "#27ae60")
                }
                className="btn-bulk btn-bulk-approve"
              >
                <i className="fa-solid fa-circle-check"></i> Phê duyệt
              </button>
              <button
                onClick={() =>
                  processBulkAction("Rejected", "Từ chối", "#e31837")
                }
                className="btn-bulk btn-bulk-reject"
              >
                <i className="fa-solid fa-circle-xmark"></i> Gỡ / Từ chối
              </button>
            </div>
          )}
          {user?.role === "Partner" && (
      ...
      <div className="filter-container">
        ...
        {user?.role === "Admin" && (
          <div className="filter-item">
            <label className="filter-label" style={{ color: "#8e44ad" }}>
              Lọc theo Đối tác:
            </label>
            <select
              className="filter-input"
              ...
              {user?.role === "Admin" && (
                <th style={{ width: "40px", textAlign: "center" }}>
                  <input
                    ...
                    checked={isAllSelected && filteredTours.length > 0}
                    className="tour-checkbox"
                    title="Chọn tất cả Tour hiển thị"
                  />
                </th>
              )}
              <th>Hình Ảnh</th>
          ...
          <tbody>
            {filteredTours.length === 0 ? (
              <tr>
                ...
                >
                  Không có Tour nào khớp với bộ lọc!
                </td>
              </tr>
            ) : (
              filteredTours.map((tour) => (
                ...
                >
                  ...
                  {user?.role === "Admin" && (
                    <td className="tour-row-creator">
                      {tour.createdBy?.fullname || "Ẩn danh"}
                    </td>
                  )}
                  <td>
                    <span
                      className={`badge-status ${tour.status === "Approved" ? "badge-approved" : tour.status === "Rejected" ? "badge-rejected" : "badge-pending"}`}
                    >
                      {tour.status === "Approved" ? (
                        <>
                          <i className="fa-solid fa-circle-check"></i> Đã Duyệt
                        </>
                      ) : tour.status === "Rejected" ? (
                        <>
                        ...
                        <>
                          <i className="fa-solid fa-hourglass-half"></i> Chờ
                          Duyệt
                        </>
                      )}
                    </span>
                  ...
                  <td>
                    <div className="action-buttons-group">
                      ...
                      {user?.role === "Admin" && tour.status !== "Approved" && (
                        <button
                          onClick={() =>
                            handleApproveTour(tour._id, "Approved")
                          }
                          className="btn-icon btn-quick-approve"
                          title="Duyệt nhanh"
                        >
                      ...
                      {user?.role === "Admin" && tour.status !== "Rejected" && (
                        <button
                          onClick={() =>
                            handleApproveTour(tour._id, "Rejected")
                          }
                          className="btn-icon btn-quick-reject"
                          title="Từ chối nhanh"
                        >
                      ...
                      {user?.role === "Partner" && (
                        <>
                          <Link
                            ...
                            className="btn-partner-action btn-edit"
                          >
                          ...
                          <button
                            ...
                            className="btn-partner-action btn-delete"
                          >
