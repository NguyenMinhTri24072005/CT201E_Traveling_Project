...
export default function Chat() {
    ...
    useEffect(() => {
        if (socket.current) {
            ...
            socket.current.on("msg-receive", (msg) => {
                setArrivalMessage({ fromSelf: false, text: msg, createdAt: new Date() });
            });
        }
    }, [socket.current]);
    ...
    if (!user) return <h2 className="login-alert-msg">Vui lòng đăng nhập để sử dụng tính năng Chat!</h2>;
                ...
                <div className="chat-filter-section">
                    ...
                    <div className="chat-tabs-container">
                        {['All', 'Customer', 'Partner', 'Admin'].map(tab => (
                            ...
                            >
                                {tab === 'All' ? 'Tất cả' : tab === 'Customer' ? 'Khách' : tab === 'Partner' ? 'Đối tác' : 'Admin'}
                            </button>
                        ))}
                    </div>
                </div>
                ...
                <div className="contact-list">
                    {filteredContacts.length === 0 ? (
                        ...
                        filteredContacts.map((contact) => (
                            ...
                            >
                                <img 
                                    src={contact.avatar && !contact.avatar.includes('default') ? `${IMAGE_URL}${contact.avatar}` : "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png"} 
                                    alt="avatar" 
                                    className="contact-avatar"
                                />
                                <div className="contact-info">
                                    <h4>{contact.fullname}</h4>
                                    <p>{contact.role === 'Partner' ? '🤝 Đối tác' : contact.role === 'Admin' ? '👑 Quản trị viên' : '🎒 Khách hàng'}</p>
                                </div>
                            </div>
                        ))
                    )}
                ...
                {currentChat === null ? (
                    ...
                    <>
                        <div className="chat-header">
                            <img 
                                src={currentChat.avatar && !currentChat.avatar.includes('default') ? `${IMAGE_URL}${currentChat.avatar}` : "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png"} 
                                alt="avatar" 
                                className="contact-avatar"
                            />
                        ...
                        <div className="chat-messages">
                            {messages.map((msg, index) => (
                                <div ref={scrollRef} key={index} className={`message ${msg.fromSelf ? 'sent' : 'received'}`}>
                                    {msg.text}
                                </div>
                            ))}
                        </div>
